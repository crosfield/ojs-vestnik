	...
	{* {fbvFormArea id="tagitFields" title="submission.submit.metadataForm"} *}
		...
		{if $citationsEnabled}
			{fbvFormSection label="submission.citations"}
				{fbvElement label="submission.citations.description" type="textarea" id="citations" multilingual=true value=$citations disabled=$readOnly required=$citationsRequired}
			{/fbvFormSection}
		{/if}
	{* {/fbvFormArea} *}
{/if}
