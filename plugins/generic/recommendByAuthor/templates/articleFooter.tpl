				...
				{iterate from=articlesBySameAuthor item=articleBySameAuthor}
					...
					<li>
						{foreach from=$article->getAuthors() item=author name=authors}
							{$author->getFullNameInitials()|escape}{if not $smarty.foreach.authors.last},{/if}
						{/foreach}
						...
						<i><a href="{url journal=$journal->getPath() page="issue" op="view" path=$issue->getBestIssueId()}">
							{$journal->getLocalizedName()|escape}</i>. {$issue->getIssueIdentification()|escape}
						</a>
					</li>
				{/iterate}
			</ul>
